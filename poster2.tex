\documentclass[30pt,extrafontsizes]{memoir}
%\setstocksize{30in}{40in}
\usepackage{xcolor}
\usepackage[paperwidth=40in,paperheight=30in,margin=1.25in]{geometry}
\pagestyle{empty}

%\usepackage[default,semibold]{sourcesanspro}
%\usepackage{sourcecodepro}
%\usepackage[T1]{fontenc}
\usepackage{fontspec}
%\setmainfont[BoldFont={Source Sans Pro Semibold}]{Source Sans Pro Light}
\setmainfont[BoldFont={Source Sans Pro Semibold}, RawFeature={+kern, +ss01}]{Source Sans Pro}

\usepackage[english]{babel}
\usepackage{blindtext}

\usepackage{pbox}
\usepackage{enumitem}
\setitemize{label={\color{cardinal}\vrule height\dimexpr.7ex+0.125ex depth\dimexpr-.7ex+0.125ex width0.5em},
            leftmargin=0pt,
            itemsep=-0.05in,
            topsep=-0.05in}
\setenumerate{label={\color{cardinal}\arabic*},
            leftmargin=0pt,
            itemsep=-0.05in,
            topsep=-0.05in}
\setlist{nosep,after=\vspace{0.4\baselineskip}}


\usepackage{multicol}
\setlength{\columnsep}{-0.00in}
\setlength{\parskip}{1.5\baselineskip}
\setlength{\parindent}{0pt}

\usepackage{microtype}
\usepackage{tcolorbox}
\definecolor{cardinal}{cmyk}{0, 1.00, 0.65, 0.34}
\definecolor{sandstone}{cmyk}{0.03, 0.12, 0.34, 0.10}
\definecolor{cloud}{cmyk}{0.03, 0.04, 0.14, 0.08}
%\definecolor{mainred}{rgb}{0, 0, 0}

%\pagecolor{cloud}

%{\hspace{1em}\Large Section Header}\\[24pt]
%\fbox{\parbox{\columnwidth}{\blindtext}}
%\parbox{\columnwidth}{\hspace{1em}{\Large Section Header}\\[18pt]\blindtext}

%\newcommand{\textbox}[2]{
  %\parbox{0.95\columnwidth}{\hspace{1em}{\Large #1}\\[18pt]#2}}
\newcommand{\textbox}[1]{
      \parbox{\dimexpr0.95\columnwidth-0.5in}{\setlength{\parskip}{0.15in}\setlength{\partopsep}{0in}#1}
    }
\newcommand{\rulebox}[2]{
  \vskip0.5in
  \parbox{0.95\columnwidth}{
    \leavevmode{\Large
    \color{cardinal}
    \leaders\hrule height\dimexpr.5ex+0.075ex depth\dimexpr-.5ex+0.075ex\hskip 0.75in
    \hskip 0.2em \hbox{\textbf{#1}} \hskip 0.0em 
    %\leaders\hrule height.1ex depth\dimexpr-.1ex+4.0pt\hfill~}\\
      \leaders\hrule height\dimexpr.5ex+0.075ex depth\dimexpr-.5ex+0.075ex \hfill\hspace*{0pt}}\\[0.20in]
      \centering
      \parbox{\dimexpr0.95\columnwidth-0.5in}{\setlength{\parskip}{0.15in}\setlength{\partopsep}{0in}#2}\\[0.25in]
    }\par}
\renewcommand{\emph}[1]{\textit{\color{cardinal}#1}}
\newcommand{\subhead}[1]{\par\hskip-.25in\textbf{\color{cardinal}#1: }}
\newcommand{\bigidea}[1]{\parbox{0.95\columnwidth}{
  %\centering\large\color{cardinal}\textbf{\textit{#1}}}\\[0.5\baselineskip]
  \centering\large\color{cardinal}\textbf{\textit{#1}}}
}
\newcommand{\authbox}[1]{
  \begin{tabular}[t]{c}
    #1
  \end{tabular}
}
\newcommand{\head}[2]{
  \newlength{\titleheight}
  \settoheight{\titleheight}{\hbox{\parbox{30in}{\centering {\HUGE \textbf{\color{cardinal} #1}\\[0.2in]}\authbox{#2}\\[0.5in]}}}
  \parbox{3in}{\raggedright \includegraphics[height=1.2\titleheight]{stanfordlogo.eps}}
  \hfill
  \parbox{30in}{\centering {\HUGE \textbf{\color{cardinal} #1}\\[0.2in]}\authbox{#2}}
  \hfill
  \parbox{3in}{\raggedleft \includegraphics[height=1.5\titleheight]{dawnlogo.pdf}}\\[0.5in]
}
\newcommand{\splitcol}[2]{
  \parbox{#1}{
    \setlength{\columnwidth}{#1}
    {#2}
  }
}
\newcommand{\splitcolh}[3]{
  \parbox[c][#2][t]{#1}{
    \setlength{\columnwidth}{#1}
    {#3}
  }
}
\newcommand{\auth}[2]{{\Large #1}\\\texttt{\large #2}}
%\newcommand{\auth}[2]{\pbox[t]{5.0in}{{\Large {~\hfill#1\hfill~}}\\\texttt{~\hfill#2\hfill~}}}
%\renewcommand\labelitemi{{\color{cardinal}\vrule height\dimexpr.7ex+0.125ex depth\dimexpr-.7ex+0.125ex width0.5em}}
\begin{document}

\head{Increasing Dynamism in Plasticine}{\auth{Yaqi Zhang}{yaqiz@stanford.edu}\and\auth{Alexander Rucker}{acrucker@stanford.edu}\and \auth{Matthew Vilim}{mvilim@stanford.edu}}\\[0.25in]
\splitcolh{0.3\columnwidth}{20in}{
  \rulebox{Background}{
    \emph{Plasticine} is a vector Coarse-Grained Reconfigurable Array:
    \begin{itemize}
        \item 6-stage, 16-lane 32-bit floating point SIMD pipelines
        \item Distributed 256-kByte memories
        \item Memory controllers support dense and sparse DRAM access
    \end{itemize}
    \vskip-.6in
    \emph{Plasticine} demonstrated up to 95x speedup and 77x performance per watt vs. a Stratix V FPGA.
  }
  \bigidea{How can we retain Plasticine's performance and efficiency while enabling new applications?}
  \rulebox{Compiler \& Mapping Flow}{
    \centering
    \includegraphics[width=1\linewidth]{figs/flow5.pdf}\\
    \vspace{20pt}
    \includegraphics[width=1\linewidth]{figs/mapping.pdf}\\
  }
}
\splitcolh{0.4\columnwidth}{20in}{
  \rulebox{Hybrid Networks}{
    \noindent Different applications have different link activation rates and fanouts:

    \centering
    \includegraphics[width=1\linewidth]{figs/link5.pdf}\\
      \bigidea{How can we improve link utilization?}
    %\textbox{
      \begin{itemize}
          \item Use static network for high-bandwidth and broadcast links
          \item Use dynamic network to encourage link sharing on low-activation links
          \item Specialize networks at different granularities
      \end{itemize}
    %}
    \noindent Area and energy scaling for switch and router on 28nm technology: \\
    \splitcol{0.5\columnwidth}{
      \centering
      \includegraphics[width=1\linewidth]{figs/switch.pdf}
    }
    \splitcol{0.35\columnwidth}{
      \footnotesize
      \begin{tabular}{cccc}
        \#VC & Buffer Slots & Area ($\mu m^2$) & Energy($fJ$/bit) \\\hline
        \input{text/router.tex}
      \end{tabular}\\
      \centering
      \small
      %Router Area and Energy Scaling \\
    }
    \noindent Network evaluation for different benchmarks:
    \includegraphics[width=1\linewidth]{figs/area_vc.pdf}\\
    \centering
    \includegraphics[width=1\linewidth]{figs/slow_down.pdf}\\
  }
  %\splitcol{0.5\columnwidth}{
    %\centering
    %\includegraphics[width=1\linewidth]{figs/switch.pdf}\\
    %\begin{tabular}{cccc}
      %\#VC & Buffer Slots & Area ($\mu m^2$) & Energy($fJ$/bit) \\\hline
      %\input{text/router.tex}
    %\end{tabular}\\
  %}
  %\splitcol{0.5\columnwidth}{
    %\textbox{
      %\includegraphics[width=1\linewidth]{figs/area.pdf}\\
    %}
  %}
}
\splitcolh{0.3\columnwidth}{20in}{
  \rulebox{Future Work}{
    Minor hardware additions to the PCU to allow fast sort:

    \centering
    \includegraphics[width=0.9\linewidth]{figs/merge_unit.pdf}\\
    \vskip-0.75in

    \raggedright
    Multi-way merge reduces memory traffic:

    \centering
    \includegraphics[width=0.7\linewidth]{figs/multi_pcu_merge.pdf}\\
    \vskip.75in

    \raggedright
    \bigidea{What's the next class of applications to target?}
    \begin{itemize}
        \item Transactional/online applications?
        \item Streaming data analytics and networking?
        \item Graph analytics?
    \end{itemize}
    \vskip.15in
    \bigidea{What advances will be necessary to target these applications?}\\[.25in]
    Improve achieved density for generic control constructs:
    \begin{itemize}
        \item Data-dependent conditionals
        \item Finite state machine-based control
        \item Parsing support
        \item Support for more complicated data structures
    \end{itemize}
  }
}

\end{document}
